  ...
  useEffect(() => {
    ...
    async function fetchHomeSections() {
      try {
        ...
        setWatchedMovieIds(new Set(watched.content.map((movie) => movie.apiMovieId)));
        setReviewedMovieIds(new Set(reviewResponse.content.map((review) => review.apiMovieId)));
        setReviews(reviewResponse.content);

        setWatchlistMovieIds(new Set(watchlistResponse.content.map((movie) => movie.apiMovieId)));
  ...
  async function handleReviewSubmit(rating: number, comment: string) {
    ...
    try {
      if (selectedReview == null) {
        const resp = await addReview(reviewMovie.id, rating, comment, reviewMovie.title);
